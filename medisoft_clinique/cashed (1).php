...
<body bgcolor="">
<h3> Les entrées Journalières encaissées(Tickets Moderateurs+privés et service non couverts par assurance )</h3>
         ...
         while($rowfac = mysqli_fetch_array($retvalfac, MYSQLI_ASSOC))
                 {
				  ...
				  echo'<th>'.$rowfac['date'].'</th>';
				                             ...
				                             $facture1 = "SELECT insurance, insured, type, quantity,unityp FROM clients, orders WHERE clients.client_id=orders.client_id AND orders.date='$date' ";

...
{	
  include('link.php');
echo'<tr><td colspan="3"><strong><em>Pour les prives</em></strong></td></tr>';	
$p1=$pa;
$p2=$pb;
...
<div style="position: absolute; background-color:#E1F0F0; width: 430px; height: 250px; overflow: auto; left: 450px; top: 113px;">
...
/*$defper = "SELECT DISTINCT period  FROM orders ORDER BY time DESC LIMIT 1";
  ...
  <tr bgcolor="#CCCCCC">
    ...
    <td colspan="2" align="center"><strong>PAID</strong></td>
...
$t=0;
$ggtot=0;
$analy = "SELECT DISTINCT item  FROM receipt WHERE date BETWEEN '$p1' AND '$p2' ORDER BY item ASC";
        $retvalanaly = mysqli_query($link, $analy);
        if(! $retvalanaly )
           {
         ...
         while($rowanaly = mysqli_fetch_array($retvalanaly, MYSQLI_ASSOC))
			...
			$j=0;	
			$qtte2=0;		
	 $analy1 = "SELECT qtty,unitp  FROM receipt WHERE  item='$item' AND date BETWEEN '$p1' AND '$p2' AND active=1";
                        $retvalanaly1 = mysqli_query($link, $analy1);
                        if(! $retvalanaly1 )
                               {
                              ...
                              while($rowanaly1 = mysqli_fetch_array($retvalanaly1, MYSQLI_ASSOC))
									 ...
									 $qtte=$rowanaly1['qtty'];
									 $qtte2+=$qtte;
									 $tot=$rowanaly1['qtty']*$rowanaly1['unitp'];
...
//$tot+=$analycost;
 ...
 echo'<tr bgcolor="#CCCCCC">
	...
	<td bgcolor="ccccc"><strong>'.$ggtot.'</strong></td>
  </tr>';
...
<div  style="  position: absolute; background-color:#E1F0F0; left: 6px; overflow:auto; top: 113px; height: 250px; width: 430px;">
      ...
      include('link.php');
		   ...
		   $clientid=0;
		    ...
		    $analy1 = "SELECT quantity, unityp,clients.client_id,insurance  FROM orders,clients WHERE orders.client_id=clients.client_id AND orders.date BETWEEN '$p1' AND '$p2' AND orders.insured=1 AND clients.insurance='$insurance' ";
                        ...
                        if(! $retvalanaly1 )
                               {
                                  die('Could not get data:5 ' . mysqli_error($link));
                               }    
...
$analy = "SELECT DISTINCT item  FROM orders WHERE date BETWEEN '$p1' AND '$p2' AND insured=0";
        $retvalanaly = mysqli_query($link, $analy);
        if(! $retvalanaly )
           {
               die('Could not get data:6 ' . mysqli_error($link));
            }    
	 ...
	 $analy1 = "SELECT quantity, unityp  FROM orders WHERE item='$item' AND date BETWEEN '$p1' AND '$p2' AND insured=0 ";
                        $retvalanaly1 = mysqli_query($link, $analy1);
                        if(! $retvalanaly1 )
                               {
                                  die('Could not get data: 7' . mysqli_error($link));
                               }    
...
$g7=$ggtot+$_SESSION['should']+$rssb+$mmi+$mediplan;
 echo'<tr bgcolor="#CCCCCC">
    <th colspan="2">TOTAL</th>

...
$date=date('Y-m-d');
$name=$_SESSION['name'];
mysqli_query ($link,"UPDATE dettes2 SET reste=$rem, paid=$pay WHERE dette_id =$dette");
//get the number
...
$a=mysqli_query ($link,"INSERT INTO receipt (receipt_id, client_id, client_name,order_id, item, unitp, qtty, date, user) 
                               VALUES ('$no', '$client_id', '$cname',0, 'Remboursement-dette', '$pay', '1', '$date', '$name')");
if(!$a){ die('Cant\'t insert data:1 '.mysqli_error($link));}
}

if(isset($_POST['mid']))
{
$mid=$_POST['mid']; 
$b=mysqli_query ($link,"DELETE FROM products WHERE prod_id='$mid'");
if(!$b){ die('Cant\'t insert data:2 '.mysqli_error($link));}
}
...
$unityp=$_POST['unityp'];
$date=date('Y-m-d');
$c=mysqli_query ($link,"INSERT INTO products (description,type,unit_price,date,insured)VALUES('$item', '$type', '$unityp', '$date', '$assure')");
if(!$c){ die('Cant\'t insert data:3 '.mysqli_error($link));}                               
}
?>
</head>
<body>
        ...
        if(! $retvalmed ){ die('Could not get data: ' . mysqli_error($link)); }               
         while($rowmed = mysqli_fetch_array($retvalmed, MYSQLI_ASSOC))
           ...
           echo '<td >'.$rowmed['nid'].'</td>';
					 ...
					 echo '<td><input type="text" size="10" name="pay" required ></td>';

...
$client = "SELECT beneficiary, percentage FROM clients WHERE client_id=$id";
...
<div class="printme" id="printMe" style="position: absolute; background-color:#FFF;  overflow: auto; height: 300px; box-shadow:0px 0px  10px 0px;  width: 230px; top: 10px; left:10px;">

<font size="-2" face="Tahoma, Geneva, sans-serif">
<center>

<img src="img/bill2.PNG" width="210" height="73" /><br />Tel:0788 215 175/TIN: 102985373<br />
...
<strong><?php echo $beneficiary ?></strong><br />
-------------------------------------<br />
<?php
         ...
         while($row = mysqli_fetch_array($retval, MYSQLI_ASSOC))
                 {
				  ...
				  $status1=$row['status'];
				   ...
				   echo '<br>('.$row['quantity'].'x'.$row['unityp'].'='.$tot.')<br><br>';
				   $tot_amount+=$tot;
				   if ($insured==0)
				      $tot_amount_not+=$tot;
					...
					if ($print==1 && $status1==0)
				       {
						   if ($insured==0)
						   {
			        	$a=mysqli_query ($link,"INSERT INTO receipt (receipt_id, client_id, client_name,order_id, item, unitp, qtty, date, user) 
                               VALUES ('$no', '$id', '$beneficiary',$order_id, '$item' , '$unitp', '$qtty', '$date', '$name')");
                if(!$a){die('Can\'t Insert data:1 '.mysqli_error($link));}
						   }
						   
						   if ($insu=='PRIVE')
						   {
			        	$b=mysqli_query ($link,"INSERT INTO receipt (receipt_id, client_id, client_name,order_id, item, unitp, qtty, date, user) 
                               VALUES ('$no', '$id', '$beneficiary',$order_id, '$item' , '$unitp', '$qtty', '$date', '$name')");
                if(!$b){die('Can\'t Insert data:2 '.mysqli_error($link));}
						   }
				       }
...
if ($print==1 && $status==0)
{
  if ($insu!='PRIVE')
  {
    $item='TM-'.$insu;
    $unitp=$patient1;
  $c=mysqli_query ($link,"INSERT INTO receipt (receipt_id, client_id, client_name,order_id, item, unitp, qtty, date, user) 
                  VALUES ('$no', '$id', '$beneficiary',0, '$item' , '$unitp', 1, '$date', '$name')");
  if(!$c){die('Can\'t Insert data:3 '.mysqli_error($link));}
  }
}
...
BY: <?php echo $name ?><br />
<hr style="border:1px solid #000;" />
Powered by <strong>DYNASOFT LTD</strong><br />
</center>
...
<?php
if ($print==0 && $status==0 && $tot_amount!=0)
{
?>
<form action="bill.php?id=<?php echo $id; ?>&insu=<?php echo $insu; ?>&date=<?php echo $date; ?>" method="post">
<input type="hidden" name="print" value="1" />
<button style="position: absolute; left: 247px; top: 3px;" class="button" ><span>PAYER</span></button>
...
<?php
if (($print==1 || $status==1) && $tot_amount!=0 )
{
?>
<button style="position: absolute; left: 241px; top:167px;" class="button" onclick="printDiv('printMe')"><span>Print</span></button>
<?php

if ($status==0)
				       
$d=mysqli_query($link,"UPDATE orders SET status ='1'  WHERE client_id =$id AND date='$date'");
if(!$d){die('Can\'t Insert data: '.mysqli_error($link));}
}
?>

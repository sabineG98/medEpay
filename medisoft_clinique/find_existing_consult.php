				   ...
				   $id=$defrowper['client_id'];
				    ...
				    $i++;
					 ...
					 if ($i%2==0)
					  ...
					  echo'<tr>';
				   echo'<form action="consultation.php" method="post" target="_parent" name="form1" >';//////////
				   echo'<td>'.$defrowper['beneficiary'].'</td>';
				   echo'<td>'.$id.'</td>';
				  // echo'<td>'.$defrowper['insurance_code'].'</td>';
				   echo'<td>'.$defrowper['categorie'].'</td>';
				  // echo'<td>'.$defrowper['chef'].'</td>';
				   echo'<td>'.$defrowper['age'].'</td>';
				   echo'<td>'.$defrowper['sex'].'</td>';
				  // echo'<td>'.$defrowper['date'].'</td>';
				   echo'<td><input type="hidden" name="code" value="'.$id.'"><input type="hidden" name="service" value="'.$service.'">

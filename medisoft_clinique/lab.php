...
mysqli_query ($link,"INSERT INTO lab_results (client_id, act, result, date) 
                               VALUES ($id,'$act','$result','$date')");
...
if(isset($_POST['date']))
{
	...
	$price = "SELECT act_id, ta,tb,tc,td,insured FROM acts WHERE act='$exam' LIMIT 1";
...
echo'<hr>';
 $products = "SELECT act,unityp FROM acts, orders WHERE  acts.act_id=orders.item_id AND type='LABORATOIRE' AND client_id=$id AND orders.date='$date' AND orders.active=1 ORDER BY time ASC ";
	...
	/*if ($post!='lab')
		 {
			    echo $i.')'.$row['act'];
				   echo '--------<strong>('.$row['unityp'].'FRW)</strong><br>'; 
		 }	*/		 
		 ...
		 $query=mysqli_query($link,"SELECT * FROM lab_results WHERE client_id='$id' AND act='$ac' AND date ='$date'");
				while($result=mysqli_fetch_array($query))
				{
				  ...
				  if (!empty($re))
				 {
				  ...
				  echo'<input type="text" size="10" name="result" style="background-color:#0FC;" placeholder="Resultat" value="'.$re.'" />';
...
<table>
   ...
   <td>
    ...
    <input type="text" id="search-box" size="50" autocomplete="off" name="exam" placeholder="Examens de labo" />

...
mysqli_query ($link,"DELETE FROM consommables WHERE id='$mid'");
}
...
mysqli_query ($link,"INSERT INTO consommables (name_consommable, unit_price, date, insured) 
                               VALUES ('$item', '$unityp', '$date', '$assure')");
...
<form action="tarif-conso.php" method="post">
         ...
         while($rowmed = mysqli_fetch_array($retvalmed, MYSQLI_ASSOC))
                 {
					 ...
					 echo '<form  action="tarif-conso.php" method="post">';
					 echo '<td width="35">'.$i.'</td>';
					 echo '<td width="400">'.$rowmed['name_consommable'].'</td>';
					 echo '<td width="100">';
					 if($rowmed['insured']=='1')
					 echo'Oui';
					 else
					 ...
					 echo '</form>';
					 echo '<td><form action="tarif-conso.php" method="post">';

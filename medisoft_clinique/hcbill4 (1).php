...
<table style="font-size:13px; border-collapse: collapse;" widtd ="0" border="1" bordercolor="#999999" cellspacing="0" cellpadding="0">
  <tr>
  <th  bgcolor="#DDEEFF" style=" text-transform:uppercase; height:40px; font-size:16px;" colspan="22">S U M M A R Y   OF  V OUC H E R S    F O R   R W A N D A  S O C I A L  S E C U R I T Y  B O A D (R S S B) / CBHI/  <?php echo ''.$facmonth.'-'.$facyear ?></th>
  </tr>
...
$tmc = "SELECT quantity FROM orders WHERE client_id=$code AND item LIKE '%COARTEM%'  AND period='$period'";
        $retval_last = mysqli_query($link, $tmc);
        if(! $retval_last )
           {
               die('Could not get data: ' . mysqli_error($link));
            }    
...
$tmc = "SELECT quantity FROM orders WHERE client_id=$code AND item LIKE '%artes%'  AND period='$period'";
        $retval_last = mysqli_query($link, $tmc);
        if(! $retval_last )
           {
               die('Could not get data: ' . mysqli_error($link));
...
$tmc = "SELECT quantity FROM orders WHERE client_id=$code AND item LIKE '%quin%'  AND period='$period'";
        $retval_last = mysqli_query($link, $tmc);
        if(! $retval_last )
           {
               die('Could not get data: ' . mysqli_error($link));
	...
	/*END a robot which verifies that a cat 1&2client has not confirmed for maralia	*/																	
																					
															     $gt=$tcons+$tlab+$timg+$thosp+$tacts+$tconso+$tmed+$tamb;
																 if ($tcons>0 && $cat<1)//cat for categorie
																 {
																 $tm=200;
...
{ 
  ...
  <tr>
    <td colspan="10" bgcolor="#DDEEFF"><strong>TOTAL</strong></td>

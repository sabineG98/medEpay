...
<em><strong>Existing Requisitions</strong></em>     
<table style="font-size:13px; border-collapse: collapse;" width="100%" border="0" cellspacing="0" cellpadding="0">    
  <tr bgcolor="">
    <td colspan="7"></td>   
  </tr>
  <tr bgcolor="#CCCCCC">
    ...
    <td><b>Print</b></td>
      ...
      <?php 
	  $i=0;
    $per = "SELECT DISTINCT req_number,comment, date, fullname  FROM req_code, internal, users WHERE req_code.facility_id=internal.Service_id AND req_code.user_id=users.id AND req_code.confirmed=1 ORDER BY req_code.req_number DESC LIMIT 500";
        ...
        if(! $retvalper )
         ...
         while($rowper = mysqli_fetch_array($retvalper, MYSQLI_ASSOC))
           ...
           echo '<td width="130" bgcolor="">'.$rowper['fullname'].'</td>';
					//  echo '<td width="130" bgcolor="">test</td>';
